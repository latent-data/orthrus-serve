...
Long-form throughput (HTTP, long prompt ~1400 tok, greedy), showing how the drafter win and the quant win stack:

| Scheme | Orthrus diffusion | TPF | AR floor (no-diff ≈ Qwen3-8B) | Drafter speedup |
|---|---:|---:|---:|---:|
| bf16 | 52.7 tok/s | 4.35 | 10.9 tok/s | 4.8× |
| fp8 (default) | 68.2 tok/s | 4.28 | 14.0 tok/s | 4.9× |
| fp8-row | 80.3 tok/s | **5.20** | 16.3 tok/s | 4.9× |
| nvfp4 | **90.1 tok/s** | 4.19 | 22.2 tok/s | 4.1× |
| nvfp4 (no-diff sanity) | — | **1.00** | 22.2 tok/s | — |

TPF is tokens-per-model-forward, a direct measure of the diffusion drafter's accept rate (counted via a forward pre-hook in [`generation.py`](src/orthrus_serve/generation.py)). Pure autoregressive generation is TPF = 1.0 by definition; the no-diff nvfp4 sanity row reads `tpf=1.00` exactly. Every diffusion arm reads ≥ 4.0, including the 4-bit one — there's no scheme where the drafter quietly stops accepting.

Generated by [`benchmarks/benchmark_http.py`](benchmarks/benchmark_http.py) (and the in-process [`benchmarks/benchmark.py`](benchmarks/benchmark.py), which agrees within 1%). The diffusion arm holds a 4.1-4.9× tok/s lead over the autoregressive floor at every quantisation; quant throughput stacks on top of that. The nvfp4 ratio is the smallest only because 4-bit weights also speed up the AR floor (22.2 tok/s, vs 16.3 at fp8), not because the drafter weakens — diffusion nvfp4 is still the fastest absolute config. Per-prompt detail, fp8/nvfp4 tool-eval-bench scores, scenario-level structure, and the single-side-quantisation probe live in [RESEARCH_LOG.md](RESEARCH_LOG.md).
